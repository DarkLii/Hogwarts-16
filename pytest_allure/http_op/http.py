...
class Http:
    ...
    def request(self, url, params=None, json_str=None, data=None, method=None):
        ...
        method = method.upper()
        if method == "GET":
            response = self.session.get(url=url, params=params, json=json_str, data=data,
                                        headers=self.headers, cookies=self.cookies, timeout=self.timeout)
        elif method == "PUT":
            response = self.session.put(url=url, params=params, json=json_str, data=data,
                                        headers=self.headers, cookies=self.cookies, timeout=self.timeout)
        elif method == "POST":
            response = self.session.post(url=url, params=params, json=json_str, data=data,
                                         headers=self.headers, cookies=self.cookies, timeout=self.timeout)
        else:
    ...
    def get(self, url, params=None, json_str=None, data=None):
        return self.request(url, params=params, json_str=json_str, data=data, method="GET")
