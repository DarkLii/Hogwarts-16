...
from test_project.service.common.common import we_weixin_login_cookie_file_path
...
class BasePage:
    ...
    def __init__(self, base_driver=None):
        ...
        if base_driver is None:
            ...
            # 获取网页版企业微信 cookie，序列化后存入yaml文件内
            self.__get_cookie()
            # 使用序列化 cookie 的方法进行登录
            self.__cookie_login()
        else:
    ...
    def __get_cookie(self):
        ...
        with open(we_weixin_login_cookie_file_path(), "w", encoding="UTF-8") as f:
            yaml.dump(cookie, f)
        print(f"获取网页版企业微信 cookie 成功：{cookie}")
